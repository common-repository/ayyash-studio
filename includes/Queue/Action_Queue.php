...
use AyyashStudio\AyyashStudioDateTime;
use Exception;
...
class Action_Queue implements Queue_Interface {
	 ...
	 *
	 * @return AyyashStudioDateTime|null The date and time for the next occurrence, or null if there is no pending, scheduled action for the given hook.
	 * @throws Exception
	 */
	public function get_next( $hook, $args = null, $group = '' ) {
		...
		if ( is_numeric( $next_timestamp ) ) {
			return new AyyashStudioDateTime( "@{$next_timestamp}", new \DateTimeZone( 'UTC' ) );
		}
