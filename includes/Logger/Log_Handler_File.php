...
class Log_Handler_File extends Log_Handler {
	...
	public function __construct( $log_size_limit = null ) {
		...
		$this->log_size_limit = apply_filters( 'ayyash_studio_log_file_size_limit', $log_size_limit );
	...
	public function clear( $handle ) {
		...
		do_action( 'ayyash_studio_log_clear', $handle );
	...
	public function remove( $handle ) {
		...
		if ( isset( $logs[ $handle ] ) && $logs[ $handle ] ) {
			$file = realpath( trailingslashit( AYYASH_STUDIO_LOG_DIR ) . $logs[ $handle ] );
			if ( 0 === stripos( $file, realpath( trailingslashit( AYYASH_STUDIO_LOG_DIR ) ) ) && is_file( $file ) && is_writable( $file ) ) { // phpcs:ignore WordPress.VIP.FileSystemWritesDisallow.file_ops_is_writable, WordPressVIPMinimum.Functions.RestrictedFunctions.file_ops_is_writable
				$this->close( $file ); // Close first to be certain no processes keep it alive after it is unlinked.
				$removed = unlink( $file ); // phpcs:ignore WordPress.VIP.FileSystemWritesDisallow.file_ops_unlink, WordPressVIPMinimum.Functions.RestrictedFunctions.file_ops_unlink
			}
			do_action( 'ayyash_studio_log_remove', $handle, $removed );
		}
		return $removed;
	}
	...
	protected function increment_log_infix( $handle, $number = null ) {
		...
		if ( is_writable( $rename_from ) ) { // phpcs:ignore WordPress.VIP.FileSystemWritesDisallow.file_ops_is_writable, WordPressVIPMinimum.Functions.RestrictedFunctions.file_ops_is_writable
			return rename( $rename_from, $rename_to ); // phpcs:ignore WordPress.VIP.FileSystemWritesDisallow.file_ops_rename, WordPressVIPMinimum.Functions.RestrictedFunctions.file_ops_rename
		} else {
			return false;
		}
	}
	...
	public static function get_log_file_path( $handle ) {
		if ( function_exists( 'wp_hash' ) ) {
			return trailingslashit( AYYASH_STUDIO_LOG_DIR ) . self::get_log_file_name( $handle );
		} else {
			ayyash_studio_doing_it_wrong( __METHOD__, __( 'This method should not be called before plugins_loaded.', 'ayyash-studio' ), '3.0' );
			return false;
		}
	}
	...
	public static function get_log_file_name( $handle ) {
		if ( function_exists( 'wp_hash' ) ) {
			...
			ayyash_studio_doing_it_wrong( __METHOD__, __( 'This method should not be called before plugins_loaded.', 'ayyash-studio' ), '3.3' );
			return false;
		}
	}
	...
	public static function delete_logs_before_timestamp( $timestamp = 0 ) {
		...
		foreach ( $log_files as $log_file ) {
			$last_modified = filemtime( trailingslashit( AYYASH_STUDIO_LOG_DIR ) . $log_file );

			if ( $last_modified < $timestamp ) {
				@unlink( trailingslashit( AYYASH_STUDIO_LOG_DIR ) . $log_file ); // @codingStandardsIgnoreLine.
			}
		}
	}
	...
	public static function get_log_files() {
		$files  = @scandir( AYYASH_STUDIO_LOG_DIR ); // @codingStandardsIgnoreLine.
		$result = array();
...
// End of file Log_Handler_File.php.
