...
class WXR_Importer extends \WP_Importer {
	...
	protected function get_reader( $file ) {
		...
		if ( ! $status ) {
			return new WP_Error( 'wxr_importer.cannot_parse', __( 'Could not open the file for parsing', 'ayyash-studio' ) );
		}
					...
					if ( version_compare( $this->version, self::MAX_WXR_VERSION, '>' ) ) {
						$this->logger->warning(
							...
							/* translators: %1$s is WXR version, %2$s is max supported WXR version. */
								__( 'This WXR file (version %1$s) is newer than the importer (version %2$s) and may not be supported. Please consider updating.', 'ayyash-studio' ),
								$this->version,
	...
	protected function import_start( $file ) {
		if ( ! is_file( $file ) ) {
			return new WP_Error( 'wxr_importer.file_missing', __( 'The file does not exist, please try again.', 'ayyash-studio' ) );
		}
	...
	public function set_user_mapping( $mapping ) {
		foreach ( $mapping as $map ) {
			if ( empty( $map['old_slug'] ) || empty( $map['old_id'] ) || empty( $map['new_id'] ) ) {
				$this->logger->warning( __( 'Invalid author mapping', 'ayyash-studio' ) );
				...
				case 'wp:status':
					...
					if ( 'auto-draft' === $data['post_status'] ) {
						...
						return new WP_Error(
							...
							__( 'Cannot import auto-draft posts', 'ayyash-studio' ),
							$data
						);
					}
					break;
	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		...
		if ( ! $post_type_object ) {
			$this->logger->warning(
				...
				/* translators: %1$s is the import message, %2$s is post type. */
					__( 'Failed to import "%1$s": Invalid post type %2$s', 'ayyash-studio' ),
					$data['post_title'],
					$data['post_type']
				)
			);
		...
		if ( $post_exists ) {
			...
			/* translators: %1$s single post type, %2$s is post title. */
				__( '%1$s "%2$s" already exists.', 'ayyash-studio' ),
			...
			if ( ! $this->options['fetch_attachments'] ) {
				$this->logger->notice(
					...
					/* translators: %s is post title */
						__( 'Skipping attachment "%s", fetching attachments disabled', 'ayyash-studio' ),
						$data['post_title']
					)
				);
				/**
		...
		if ( is_wp_error( $post_id ) ) {
			$this->logger->error(
				...
				/* translators: %1$s is the post title, %2$s is post type. */
					__( 'Failed to import "%1$s" (%2$s)', 'ayyash-studio' ),
					$data['post_title'],
					$post_type_object->labels->singular_name
				)
			);
		...
		$this->logger->info(
			sprintf(
			/* translators: %1$s is the post title, %2$s is post type. */
				_x( 'Imported "%1$s" (%2$s)', 'Process Post', 'ayyash-studio' ),
				$data['post_title'],
				$post_type_object->labels->singular_name
			)
		);
		$this->logger->debug(
			...
			/* translators: %1$s is the original post id, %2$s is old post id. */
				__( 'Post %1$d remapped to %2$d', 'ayyash-studio' ),
				$original_id,
				$post_id
			)
		);
	...
	protected function process_attachment( $post, $meta, $remote_url ) {
		...
		if ( ! $info ) {
			return new WP_Error( 'attachment_processing_error', __( 'Invalid file type', 'ayyash-studio' ) );
		}
	...
	protected function process_author( $data, $meta ) {
		...
		if ( is_wp_error( $user_id ) ) {
			$this->logger->error(
				...
				/* translators: %s user login name  */
					__( 'Failed to import user "%s"', 'ayyash-studio' ),
					$userdata['user_login']
				)
			);
		...
		$this->logger->info(
			sprintf(
			/* translators: %s user login name  */
				__( 'Imported user "%s"', 'ayyash-studio' ),
				$userdata['user_login']
			)
		);
		$this->logger->debug(
			...
			/* translators: %1$s original user Id, %2$s old user Id.  */
				__( 'User %1$d remapped to %2$d', 'ayyash-studio' ),
				$original_id,
				$user_id
			)
		);
	...
	protected function process_term( $data, $meta ) {
		...
		$this->logger->info(
			...
			/* translators: %1$s is the taxonomy name, %2$s is taxonomy. */
				_x( 'Imported "%1$s" (%2$s)', 'Process Taxonomy Term', 'ayyash-studio' ),
				$data['name'],
				$data['taxonomy']
			)
		);
		$this->logger->debug(
			...
			/* translators: %1$s is term original id, %2$s is term id. */
				__( 'Term %1$d remapped to %2$d', 'ayyash-studio' ),
				$original_id,
				$term_id
			)
		);
	...
	protected function fetch_remote_file( $url, $post ) {
		...
		if ( 200 !== $code ) {
			...
			return new WP_Error(
				...
				/* translators: %1$s is error code, %2$s is error code header, %3$s is url. */
					__( 'Remote server returned %1$d %2$s for %3$s', 'ayyash-studio' ),
					$code,
		...
		if ( isset( $headers['content-length'] ) && $filesize !== (int) $headers['content-length'] ) {
			unlink( $upload['file'] );
			return new WP_Error( 'import_file_error', __( 'Remote file is incorrect size', 'ayyash-studio' ) );
		}

		if ( 0 === $filesize ) {
			unlink( $upload['file'] );
			return new WP_Error( 'import_file_error', __( 'Zero size file downloaded', 'ayyash-studio' ) );
		}
		...
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			...
			$message = sprintf( __( 'Remote file is too large, limit is %s', 'ayyash-studio' ), size_format( $max_size ) );
		...
		foreach ( $todo as $post_id => $_ ) {
			$this->logger->debug(
				...
				/* translators: %d is post id. */
					__( 'Running post-processing for post %d', 'ayyash-studio' ),
					$post_id
				)
			);
				...
				if ( isset( $this->mapping['post'][ $parent_id ] ) ) {
					...
					$this->logger->warning(
						...
						/* translators: %1$s is post title, %2$s is post id. */
							__( 'Could not find the post parent for "%1$s" (post #%2$d)', 'ayyash-studio' ),
					...
					$this->logger->debug(
						...
						/* translators: %1$d is post id, %2$d is parent post id. */
							__( 'Post %1$d was imported with parent %2$d, but could not be found', 'ayyash-studio' ),
							$post_id,
							$parent_id
						)
					);
				...
				if ( isset( $this->mapping['user_slug'][ $author_slug ] ) ) {
					...
					$this->logger->warning(
						...
						/* translators: %1$s is the post title, %2$s is post id. */
							__( 'Could not find the author for "%1$s" (post #%2$d)', 'ayyash-studio' ),
							get_the_title( $post_id ),
							$post_id
						)
					);
					$this->logger->debug(
						...
						/* translators: %1$d is post id, %2$s is author slug. */
							__( 'Post %1$d was imported with author "%2$s", but could not be found', 'ayyash-studio' ),
							$post_id,
							$author_slug
						)
					);
			...
			if ( empty( $data ) ) {
				$this->logger->debug(
					sprintf(
					/* translators: %d is post id. */
						__( 'Post %d was marked for post-processing, but none was required.', 'ayyash-studio' ),
						$post_id
					)
				);
				continue;
			...
			if ( is_wp_error( $result ) ) {
				$this->logger->warning(
					sprintf(
					/* translators: %1$s is the post title, %2$s is post id. */
						__( 'Could not update "%1$s" (post #%2$d) with mapped data', 'ayyash-studio' ),
						get_the_title( $post_id ),
						$post_id
					)
				);
	...
	protected function post_process_menu_item( $post_id ) {
		...
		if ( ! empty( $menu_object ) ) {
			...
			$this->logger->warning(
				sprintf(
				/* translators: %1$s is the post title, %2$s is post id. */
					__( 'Could not find the menu object for "%1$s" (post #%2$d)', 'ayyash-studio' ),
					get_the_title( $post_id ),
					$post_id
				)
			);
			$this->logger->debug(
				...
				/* translators: %1$s is post id, %2$s is post object id, %3$s is menu type. */
					__( 'Post %1$d was imported with object "%2$d" of type "%3$s", but could not be found', 'ayyash-studio' ),
					$post_id,
					$menu_object_id,
					$menu_item_type
				)
		...
		foreach ( $todo as $comment_id => $_ ) {
				...
				if ( isset( $this->mapping['comment'][ $parent_id ] ) ) {
					...
					$this->logger->warning(
						...
						/* translators: %d is comment id. */
							__( 'Could not find the comment parent for comment #%d', 'ayyash-studio' ),
							$comment_id
						)
					);
					$this->logger->debug(
						...
						/* translators: %1$s is comment id, %2$s is parent comment id. */
							__( 'Comment %1$d was imported with parent %2$d, but could not be found', 'ayyash-studio' ),
							$comment_id,
							$parent_id
						)
					);
			...
			if ( ! empty( $author_id ) ) {
				...
				if ( isset( $this->mapping['user'][ $author_id ] ) ) {
					...
					$this->logger->warning(
						sprintf(
						/* translators: %d is comment id. */
							__( 'Could not find the author for comment #%d', 'ayyash-studio' ),
							$comment_id
						)
					);
					$this->logger->debug(
						...
						/* translators: %1$d is comment id, %2$d is author id. */
							__( 'Comment %1$d was imported with author %2$d, but could not be found', 'ayyash-studio' ),
							$comment_id,
							$author_id
						)
					);
			...
			if ( empty( $result ) ) {
				$this->logger->warning(
					sprintf(
					/* translators: %d is comment id. */
						__( 'Could not update comment #%d with mapped data', 'ayyash-studio' ),
						$comment_id
					)
				);
				continue;
	...
	protected function prefill_existing_terms() {
		global $wpdb;
		$query = "SELECT t.term_id, tt.taxonomy, t.slug FROM $wpdb->terms AS t JOIN $wpdb->term_taxonomy AS tt ON t.term_id = tt.term_id";
